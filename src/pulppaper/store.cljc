(ns pulppaper.store
  "SSoT for the pulp/paper mill plant-operations coordination actor,
  ...
  for dev/tests/demo, no deps. Mirroring `cloud-itonami-isic-1610`'s
  own sawmilling vertical (docs/adr/0001-architecture.md Decision 1),
  this vertical is self-contained (no external pulppaper capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             pulp/paper-grade/volume/brightness
                             ...
                             `:shipped-volume-tonnes` tracks the
                             batch's own cumulative-shipped ground
                             truth.
    - `equipment`         -- a digester/paper-machine/effluent-
                             treatment-plant unit's own record.
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled digester/paper-machine/
                             effluent-treatment maintenance-window
                             DRAFT against a piece of equipment
                             (`pulppaper.registry`'s
    ...
    - `shipments`         -- a proposed outbound pulp/paper/paperboard-
                             shipment DRAFT (`pulppaper.registry`'s
                             `register-shipment`).
  ...
  audit trail a mill owner or pulp/paper buyer trusting this
  coordinator needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (pulppaper.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (pulppaper.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :grade :bleached-kraft-pulp :species "Southern Pine"
                :volume-tonnes 500.0 :brightness-percent 88.0
                :verified? true :registered? true
                :shipped-volume-tonnes 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :grade :newsprint :species "Recycled Fiber Furnish"
                :volume-tonnes 80.0 :brightness-percent 58.0
                :verified? true :registered? true
                :shipped-volume-tonnes 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :grade :kraft-paper :species "Loblolly Pine"
                :volume-tonnes 600.0 :brightness-percent 45.0
                :verified? false :registered? false
                :shipped-volume-tonnes 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :digester
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "equip-002" {:id "equip-002" :kind :effluent-treatment-plant
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `pulppaper.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `pulppaper.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
      ...
      (let [shipment-id (first path)
        ...
        (swap! a (fn [state]
                   (-> state
                       ...
                       (update-in [:batches batch-id :shipped-volume-tonnes]
                                  (fn [prev]
                                    (+ (double (or prev 0.0))
                                       (double (or (:volume-tonnes value) 0.0))))))))
        result)
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered digester unit
  (schedulable for maintenance), one UNVERIFIED/unregistered effluent-
  treatment-plant unit (blocks any maintenance scheduling against it)
  -- so the actor + demo + tests run offline. Returns `s` (thread-
  friendly with `->`)."
  [s]
