(ns pulppaper.advisor
  "PulpPaperAdvisor -- the *contained intelligence node* for the
  pulp/paper mill plant-operations coordination actor.

  It normalizes production-batch patches (pulp/paper-grade/volume/
  brightness), drafts a digester/paper-machine/effluent-treatment
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound pulp/paper/paperboard
  ...
  a real pulping/paper-machine-equipment actuation, effluent-discharge
  authorization, or freight dispatch. Every output is censored
  downstream by `pulppaper.governor` before anything touches the SSoT
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct pulping/
                                 ; paper-machine-equipment-control or
  ...
  contract every caller of this actor agrees to) -- `pulppaper.governor`
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [pulppaper.registry :as registry]
...
(defn- log-production-batch
  ...
  brightness or verification status. High confidence, low stakes --
...
(defn- schedule-maintenance
  "Draft a digester/paper-machine/effluent-treatment maintenance-
  ...
  rationale, but `pulppaper.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a chemical-hazard/effluent-discharge/equipment safety-concern
  flag. ALWAYS `:stake :coordination/safety-concern` -- a safety
  ...
  technicality). See `pulppaper.phase`: no phase ever adds this op to
  a phase's `:auto` set; `pulppaper.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound pulp/paper/paperboard shipment coordination
  proposal against a production batch. The advisor passes through the
  caller's own claimed volume -- it does NOT invent one, and
  `pulppaper.governor` NEVER trusts it: it independently recomputes
  whether the batch's own cumulative-shipped volume plus this claim
  would exceed the batch's own recorded volume before any commit is
  possible."
  [db {:keys [subject value]}]
  (let [batch-id (:batch-id value)
        ...
        over-volume? (and b (registry/shipment-volume-exceeded?
                             b (:volume-tonnes value)))]
    {:summary    (str subject " 向け出荷調整提案 ("
                      (:volume-tonnes value) " トン)"
...
(def ^:private system-prompt
  (str "あなたはパルプ・製紙工場プラント運用コーディネーターの助言者です。"
       ...
       "排水放流の許可(discharge-authorize)確定や蒸解釜・抄紙機等の設備の直接操作を絶対に"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `pulppaper.governor`
