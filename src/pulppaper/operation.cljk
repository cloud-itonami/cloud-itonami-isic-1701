(ns pulppaper.operation
  "PulpPaperOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (PulpPaperAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Pulp & Paper
...
(defn build
  "Compiles a PulpPaperOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `pulppaper.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PulpPaperAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Pulp & Paper Plant Operations Governor -- independent censor.
      (g/add-node :govern
