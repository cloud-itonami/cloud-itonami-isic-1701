(ns pulppaper.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control pulping/paper-machine equipment
  directly... does NOT authorize an effluent discharge') implemented
  faithfully. The single invariant under test:

    PulpPaperAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Pulp & Paper Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:grade :bleached-kraft-pulp}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :bleached-kraft-pulp (:grade (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-paper-machine :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest invalid-brightness-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:brightness-percent 250.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-brightness} (-> (store/ledger db) last :basis)))
    (is (not= 250.0 (:brightness-percent (store/batch db "batch-001"))) "fabricated brightness never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "equip-001" :severity :moderate
                                            :description "chemical odor near digester"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
