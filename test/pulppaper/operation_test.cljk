(ns pulppaper.operation-test
  "Smoke tests for the compiled PulpPaperOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `pulppaper.governor-contract-test`; the Store contract in
  `pulppaper.store-contract-test`."
...
(deftest test-actor-builds
  (testing "PulpPaperOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "equip-001" :severity :moderate :description "chemical odor near digester"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
