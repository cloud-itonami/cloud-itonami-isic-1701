(ns pulppaper.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `pulppaper.store` ns docstring for why a second (Datomic-backed)
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:grade :bleached-kraft-pulp}})
    (is (= :bleached-kraft-pulp (:grade (store/batch s "batch-001"))))
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-volume
  (let [s (seeded)]
    ...
    (is (= 150.0 (:shipped-volume-tonnes (store/batch s "batch-001")))
